...
rule trim_galore_se:
    ...
    output:
        trimmed_fq = expand("{data_dir}/trimmed/trim_galore/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}/{{accession}}_trimmed.fq", data_dir=config["data"]["dir"]),
        fastqc_reports = expand("{rep_dir}/fastqc/post-trim/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}/{{accession}}_trimmed_fastqc.{suf}", rep_dir=config["reports_dir"], suf=["html", "zip"]),
        trim_reports = expand("{rep_dir}/trim_galore/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}/{{accession}}.fastq_trimming_report.txt", rep_dir=config["reports_dir"])
    ...
    params:
        output_dir=expand("{data_dir}/trimmed/trim_galore/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}", data_dir=config["data"]["dir"]),  # trimmed files and reports will be saved in this directory
        user_args=config["prep_args"]["trim_galore_se"],  # user args can be adjusted in the config file
        fastqc_rep_dir = expand("{rep_dir}/fastqc/post-trim/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}", rep_dir=config["reports_dir"]),  # fastqc reports will be moved to this directory
        trim_rep_dir = expand("{rep_dir}/trim_galore/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}", rep_dir=config["reports_dir"])  # trim reports will be moved to this directory
...
rule trim_galore_pe:
    ...
    output:
        trimmed_fq = expand("{data_dir}/trimmed/trim_galore/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}/{{accession}}_{read}_trimmed.fq", data_dir=config["data"]["dir"], read=["1", "2"]),
        fastqc_reports = expand("{rep_dir}/fastqc/post-trim/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}/{{accession}}_{read}_trimmed_fastqc.{suf}", rep_dir=config["reports_dir"], read=["1", "2"], suf=["html", "zip"]),
        trim_reports = expand("{rep_dir}/trim_galore/{{ref}}/{{patient_id}}/{{group}}-{{srx_id}}-{{layout}}/{{accession}}_{read}.fastq_trimming_report.txt", rep_dir=config["reports_dir"], read=["1", "2"])
