...
rule wgbstools_convert_bam_to_beta:
    ...
    input:
        bam = expand("{data_dir}/05_merged_sambamba/{{ref}}-{{patient_id}}-{{group}}-{{srx_id}}-{{layout}}_merged.bam", data_dir=config["data"]["dir"]), # sambamba_merge output
    ...
    output:
        expand("{data_dir}/06_wgbstools_betas/{{ref}}-{{patient_id}}-{{group}}-{{srx_id}}-{{layout}}_merged.{suf}", data_dir=config["data"]["dir"], suf=["pat.gz", "pat.gz.csi", "beta"])
    log:
    ...
    params:
        outdir = expand("{data_dir}/06_wgbstools_betas/", data_dir=config["data"]["dir"]),
        tempdir = expand("{data_dir}/06_wgbstools_betas/temp", data_dir=config["data"]["dir"]),
