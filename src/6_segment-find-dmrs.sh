...
/usr/bin/time -o ~/benchmarking/6_segment-find-dmrs/segment.txt --append -f "%C,%E,%P,%K,%x" wgbstools segment --betas *beta --min_cpg 5 --max_bp 2000 -o blocks.bed

# compress bed file and generate corresponding index file for visualization steps
/usr/bin/time -o ~/benchmarking/6_segment-find-dmrs/index.txt --append -f "%C,%E,%P,%K,%x" wgbstools index blocks.bed

# collapse beta files to homogenously methylated chunks found in segmentation step and writes to csv
/usr/bin/time -o ~/benchmarking/6_segment-find-dmrs/beta2table.txt --append -f "%C,%E,%P,%K,%x" wgbstools beta_to_table blocks.bed.gz --betas *beta | column -t > meth-segments.csv
...
/usr/bin/time -o ~/benchmarking/6_segment-find-dmrs/find-dmrs.txt --append -f "%C,%E,%P,%K,%x" wgbstools find_markers --blocks_path blocks.bed.gz --betas *beta --groups_file groups.csv --delta_quants .3 --pval 1
