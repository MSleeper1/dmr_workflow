...
rule multiqc_compile_reports:
    input:
        ...
        # expand("{rep_dir}/04_bismark_deduplication/{sample.ref}/{sample.patient_id}-{sample.group}-{sample.srx_id}-{sample.layout}/{sample.accession}_bismark_deduplication_report.txt", sample=sample_info.itertuples(), rep_dir=config["reports_dir"]), # bismark_deduplication output report
