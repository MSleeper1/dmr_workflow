...
# OBSERVED RESOURCE USAGE: 13-20 gb and 4-10 hours per pair of 10-20 gb fastq files processed

#-----------------------------------------------------------------------------------#
# description and usage
#-----------------------------------------------------------------------------------#
# 2_paired-end-alignment.sh
# Created: 4/21/23 MS
# Modified: 7/25/23 MS # added timing element
#
# this script will align paired fastq files to a reference and convert resulting sam file to bam file
#
...
#
# sbatch 2_paired-end-alignment.sh -w [directory/path] -o [folder-name] -r [reference-genome] [accession-num-1] [accession-num-2] [accession-num-3]...
#
#-----------------------------------------------------------------------------------#
...
for i; do
    ...
    /usr/bin/time -o ~/benchmarking/2_paired-end-alignment/bwa_align.txt --append -f "%C,%E,%P,%K,%x" bwameth.py --reference $reference  $i"_1.fastq" $i"_2.fastq" > $outfolder/$i.sam

    # converting sam to bam
    echo "converting $i.sam to $i.bam"
    /usr/bin/time -o ~/benchmarking/2_paired-end-alignment/samtools_convert.txt --append -f "%C,%E,%P,%K,%x" samtools view -S -b $outfolder/$i.sam > $outfolder/$i.bam
